...
//	 	axisCamera=CameraServer.getInstance().addAxisCamera("cam1", "10.7.8.11");
	    ...
	    visionThreadLiftGear = new VisionThread(usbCameraLiftGear, new GripPipelineLiftGear(), lgPipeline -> {
	    	liftPipelineSize = lgPipeline.filterContours0Output().size();
	    	gearPipelineSize = lgPipeline.filterContours1Output().size();
	        ...
	        if (!lgPipeline.filterContours0Output().isEmpty()) {
	        	
	        	for (int i = 0; i < lgPipeline.filterContours0Output().size(); i++) {
	        		Rect lift = Imgproc.boundingRect(lgPipeline.filterContours0Output().get(i));
...
//	        	}
	        ...
	        if (!lgPipeline.filterContours1Output().isEmpty()) {
  	        	
	  	       	for (int i = 0; i < lgPipeline.filterContours1Output().size(); i++) {
	  	       		Rect gear = Imgproc.boundingRect(lgPipeline.filterContours1Output().get(i));
